...
function addDetails(symbols, keys, i) {
  ...
  var description = '<p id="hiragana-lead-'+keys[i]+'"><strong><em>'+keys[i]+'</em> - </strong></p>';

  $("#kana-learn-wrapper").append(table+description);
  ...
  .done(function(data) {
    if(data !== '404') {
      $("#hiragana-lead-"+keys[i]+" > strong").append(data);
    }
  });
