...
function addSelectionRows () {
  ...
  for (var i=0; i<kanaLevelSymbols.length; i++) {
    ...
    $("#kanaSelectForm").append('<label class="kana-selector-label"><input type="checkbox" class="kana-selector-checkbox" value="'+i+'">'+symbols+'</label>');
  }
}

function selectAll () {
  $(".kana-selector-checkbox").attr('checked', 'checked');
}

function deselectAll () {
  $(".kana-selector-checkbox").removeAttr('checked');
}
...
function validateSelection () {
  ...
  sessionStorage.setItem("area", JSON.stringify(_AREA));  // area stores 'hiragana' or 'katakana' respectively
