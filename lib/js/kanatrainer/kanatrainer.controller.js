...
function initiate (area, levels) {
  ...
  for (var level in levels) {
    amountOfSymbols += _kanaHandler.getSymbolsFromLevel(level).length;
  }
  if (amountOfSymbols<4 && sessionStorage.getItem('multipleChoice')) {
    sessionStorage.removeItem('multipleChoice');
    alert('Not enough symbols for multiple choice!');
  }
