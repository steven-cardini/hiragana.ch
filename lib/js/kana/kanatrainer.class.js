...
var KanaTrainer = function (levels, kanaHandler) {
  ...
  var loadResults = function () {
    var area = getArea();
    if (!sessionStorage.getItem(area+"-results")) {
      return; // no persisted data present
    }

    var persistedData = JSON.parse(sessionStorage.getItem(area+'-results'));
  ...
  this.getTotalQuestionAmount = function () {
  ...
  this.save = function () {
    ...
    sessionStorage.setItem(area+"-results", JSON.stringify(persistedData));
  };
